...
int main (int argc, char** argv)
{
  ...
  // showCloudsLeft(transformed_cloud, stereo_cloud);
   ...
   // KdTree with 5 random trees
	...
	int64 t_agb = cv::getTickCount();
	kdtree_flann.knnSearch(stereo_points_flann, indices_flann, dists_flann, 1, flann::SearchParams());
	t_agb = cv::getTickCount() - t_agb;
	printf("Time elapsed for Non-OpenCV FLANN: %fms\n", t_agb*1000/cv::getTickFrequency());


	for(int ot=0;ot<indices_flann.size();++ot){
		cout << "(index,dist):";
		for(int in=0;in<indices_flann.at(ot).size();in++){
			cout<<"("<<indices_flann.at(ot).at(in)<<","<<dists_flann.at(ot).at(in)<<")"<<endl;
		}
	}
	...
	printf("Time elapsed for OpenCV FLANN: %fms\n", t2*1000/cv::getTickFrequency());
